...
export const problemState = selectorFamily<ProblemDto[], number>({
    key: 'problemState',
    get: (key) => async ({get}) => {

        const includes = new Set<number>();
        const result: ProblemDto[] = [];
        const amount = get(amountState(key));
        const params = {
            page: 0,
            query: get(searchQueryState(key)),
            direction: get(directoinState(key)),
            sort: get(sortState(key)),
        }
